...
@tool
def run_self_diagnostics(module: str = "", smoke: bool = False) -> str:
    """运行自身诊断，检查 Agent 各模块健康状态，生成诊断报告和自愈建议。

    覆盖：工具完整性、记忆健康、目标健康、磁盘空间、Git状态、飞书连接、快照数量。

    Args:
        module: 指定诊断模块名，如 tool_integrity, memory_health 等。留空运行全部。
        smoke: True 则只运行快速健康检查（<5秒），默认 False
    """
    import json as _json
    ...
    if smoke:
        ...
        check_map = {
            ...
            "snapshot_count": diag.check_snapshot_count,
        }
        if module not in check_map:
            return f"未知模块: {module}\n可用: {', '.join(check_map.keys())}"
        try:
            result = check_map[module]()
            report = diag._aggregate({module: result})
        except Exception as e:
            return f"诊断 {module} 失败: {str(e)}"
    else:
        report = diag.run_all()
    ...
    lines = [
        f"📊 {report['verdict']}",
        f"   总计: {report['summary']['total']} | ✅ {report['summary']['ok']} | ⚠️ {report['summary']['warn']} | ❌ {report['summary']['error']}",
        f"",
    ]
...
def health_check() -> str:
    """快速健康检查（<5秒），确认 Agent 基础运转正常。仅检查最关键的三个维度：
    工具完整性、磁盘空间、飞书连接状态。
    """
    from agent.self_diagnostics import get_diagnostics
    diag = get_diagnostics()
    ...
    lines = [
        f"🩺 {report['verdict']}",
        f"   ✅ {report['summary']['ok']} | ⚠️ {report['summary']['warn']} | ❌ {report['summary']['error']}",
    ]
    for name, check in report["checks"].items():
        icon = {"ok": "✅", "warn": "⚠️", "error": "❌"}.get(check["status"], "❓")
        lines.append(f"  {icon} {name}: {check['message']}")
...
@tool
async def check_llm_balance() -> str:
    """查询 DeepSeek API 账户余额和 token 消耗信息。

    返回当前 API Key 对应的账户余额、是否可用、已消耗 token 等信息。
    支持 DeepSeek 及兼容 OpenAI 接口的余额查询。
    """
    try:
        import httpx
    except ImportError:
        return "❌ 缺少 httpx 库，请运行: pip install httpx"
    ...
    if not api_key:
        return "❌ 未配置 API Key，请在 Settings 页面设置。"

    if not base_url:
        return "❌ 未配置 Base URL。"

    # 余额端点在根路径 /user/balance，需去掉 /v1 后缀
    root_url = base_url.rstrip("/")
    if root_url.endswith("/v1"):
        root_url = root_url[:-3]

    balance_url = f"{root_url}/user/balance"
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            # 可用状态
            is_available = data.get("is_available", data.get("isAvailable", None))
            if is_available is not None:
                lines.append(f"   状态: {'✅ 可用' if is_available else '❌ 余额不足'}")
            ...
            return (
                f"❌ 余额查询接口不存在 (404)\n"
                f"   URL: {balance_url}\n"
                f"   当前 API 可能不支持余额查询（非 DeepSeek 或接口已变更）"
            )
        else:
...
def discover_capabilities() -> str:
    """统一能力发现入口——返回 Agent 所有可用能力的结构化清单。

    这是 MCP（Model Context Protocol）式的工具发现：
    调用者无需记住具体工具名，先 discover_capabilities 查看所有能力，
    再选择合适的工具去使用。

    返回：工具清单（按类别分组）、子Agent列表、记忆系统、命令系统、影子系统状态。
    """
    import json as _json
    ...
    lines.append(f"## 🔧 工具（{len(all_tools)} 个）")
    ...
    agents = factory.list_all()
    if agents:
        lines.append(f"## 🤖 子Agent（{len(agents)} 个）")
        for a in agents:
            status_icon = {"idle": "💤", "running": "🟢", "completed": "✅", "error": "❌"}.get(a.status, "❓")
            tool_label = ', '.join(a.tools) if a.tools else '全部工具'
            lines.append(f"  {status_icon} {a.name} ({a.id}) — 工具: {tool_label}")
        lines.append("")
    else:
        lines.append(f"## 🤖 子Agent：0 个（使用 create_sub_agent 创建）")
        lines.append("")
    
    # ── 记忆系统 ──
    mem = _get_memory()
    total_mem = len(mem.store.entries)
    categories = mem.store.get_all_categories()
    tags_list = mem.store.get_all_tags()
    lines.append(f"## 🧠 持久记忆（{total_mem} 条）")
    lines.append(f"  类别: {', '.join(sorted(categories)) if categories else '无'}")
    lines.append(f"  标签: {', '.join(sorted(tags_list)[:10])}{'...' if len(tags_list) > 10 else ''}")
    lines.append("")
    ...
    lines.append(f"## 🎯 进化目标（{len(goals)} 个，{len(active_goals)} 活跃）")
    for g in active_goals[:5]:
        lines.append(f"  {'★' * g.priority} {g.title} (进度:{g.progress}%)")
    lines.append("")
    
    # ── 飞书命令 ──
    lines.append("## 💬 飞书命令系统")
    lines.append("  /status · /tools · /goals · /diagnose · /restart · /memory · /exec · /help")
    lines.append("")
    
    # ── 影子系统 ──
    lines.append("## 🔷 影子系统（蓝绿部署）")
    lines.append("  start_shadow_service · test_shadow_service · promote_shadow_service · stop_shadow_service")
    lines.append("")
    
    # ── 快照安全网 ──
    mgr = get_snapshot_manager()
    snaps = mgr.list_snapshots()
    lines.append(f"## 📸 沙盒快照（{len(snaps)} 个）")
    for s in snaps[:3]:
        lines.append(f"  {s.id} — {s.name} ({s.total_size/1024:.1f}KB)")
    lines.append("")
    
    lines.append("💡 调用任何工具前，先用 discover_capabilities 了解有哪些可用能力。")
...
# 所有工具
_ALL_TOOLS = _BUILTIN_TOOLS + _GOAL_TOOLS + _SEARCH_TOOLS + _MEMORY_TOOLS + _SHORT_TERM_MEMORY_TOOLS + _AGENT_FACTORY_TOOLS + _EVOLUTION_TOOLS + _SNAPSHOT_TOOLS + _SHADOW_TOOLS + _TEST_TOOLS + _DIAGNOSTICS_TOOLS + _DISCOVERY_TOOLS
